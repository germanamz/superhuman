...
After post-implementation verification, the planning agent posts a completion-seal note on the Story summarizing the implementation outcome, and archives each phase-plan note (`tusk_node_modify <note-path> --prop archived=true`). Archived notes remain queryable but are hidden from default `archived:false` views — they record how the Story was implemented without cluttering future context.
...
8. **Tag all bridge code with a removal target.**
   Whenever a phase introduces bridge code, tag it in the phase-plan note's Bridge Code table with the specific phase number where it will be replaced. The removal must appear as an explicit task in the target phase's note and as a task node in that phase. If you cannot name a removal phase, the plan is incomplete.

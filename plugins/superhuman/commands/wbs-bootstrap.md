---
description: Initialize the current repo as a Tusk workspace and install the superhuman-wbs pack so /wbs-new, /wbs-status, and the wbs-orientation skill can run.
argument-hint: [name=<workspace-name>]
---

# /wbs-bootstrap

Bootstrap the current working directory for the Superhuman WBS flow on Tusk v1. Idempotent: safe to re-run. Initializes a Tusk workspace (if one isn't already present) and adds the `superhuman-wbs` pack from this plugin's `packs/wbs.toml`. Prints a what-to-do-next message and a one-time hint if `[embeddings]` isn't configured.
...
3. **Detect existing pack.** Read `tusk.toml` and check for `[node-types.wbs-node]`. If present, the pack is already loaded — skip step 4 and emit a soft note: "superhuman-wbs pack already present; skipping `tusk pack add`."

4. **Add the superhuman-wbs pack.** Resolve the absolute path to this plugin's `packs/wbs.toml`. Inside a slash command, `${CLAUDE_PLUGIN_ROOT}` resolves to the plugin's install root, so the pack lives at `${CLAUDE_PLUGIN_ROOT}/packs/wbs.toml`. Run:
...
7. **Print what to do next.** Emit a short message scoped to the resulting state:
   ...
   > - Level templates: `plugins/superhuman/templates/wbs/desc-<level>.md`.
...
- **`${CLAUDE_PLUGIN_ROOT}/packs/wbs.toml` not found** — hard error. Indicates a corrupted plugin install; recommend reinstalling the `superhuman` plugin from the marketplace.
